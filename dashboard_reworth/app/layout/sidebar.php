...
function sidebar_items(string $role): array
{
    return match ($role) {
        ...
        'seller' => [
            ...
            ['type' => 'link', 'key' => 'saldo', 'label' => 'Saldo', 'path' => 'app/modules/seller/transactions.php', 'icon' => 'wallet'],
...
function sidebar_item_active(string $role, string $key, string $currentPath, array $query): bool
{
    if ($role === 'admin') {
        ...
        return match ($key) {
            ...
            'data_masyarakat' => str_ends_with($currentPath, 'app/modules/admin/users.php'),
            'data_seller' => (str_ends_with($currentPath, 'app/modules/admin/sellers.php') && !isset($query['status'])) || ($isSellerDetail && !$isPengajuan && $sellerSource !== 'verification'),
            ...
            'verifikasi_seller' => ($isSellerDetail && ($sellerSource === 'verification' || $isPengajuan)) || (str_ends_with($currentPath, 'app/modules/admin/sellers.php') && ($query['status'] ?? '') === 'menunggu'),
...
function render_sidebar(array $user): void
{
    ...
    $displayName = trim((string) (($user['nama_lengkap'] ?? $user['nama'] ?? '') ?: 'Pengguna ReWorth'));
        ...
        <div class="sidebar-footer">
            <a class="sidebar-link sidebar-logout-link" href="<?= e(url($logout['path'])) ?>" data-confirm="<?= e($logout['confirm']) ?>">
                <span class="sidebar-group-main">
                    ...
                    <span class="sidebar-label"><?= e($logout['label']) ?></span>
                </span>
            </a>
